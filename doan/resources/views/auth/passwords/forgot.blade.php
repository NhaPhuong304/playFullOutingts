...
<head>
    ...
    <title>Quên Mật Khẩu - PlayFullOutings</title>
...
<p class="text-[#111811] dark:text-white text-4xl font-black leading-tight tracking-[-0.033em]">Quên Mật Khẩu?</p>
<p class="text-[#618961] dark:text-gray-300 text-base font-normal leading-normal">Không sao! Nhập email của bạn và chúng tôi sẽ gửi mã xác nhận.</p>
</div>
</div>
<div class="w-full mt-8">
<form class="flex flex-col gap-6" method="POST" action="{{ route('password.forgot.post') }}">
    @csrf
    @if($errors->any())
...
<p class="text-[#111811] dark:text-white text-base font-medium leading-normal pb-2">Email</p>
...
<input name="email" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-l-xl text-[#111811] dark:text-white focus:outline-0 focus:ring-0 border-0 bg-transparent h-14 placeholder:text-[#618961] dark:placeholder:text-gray-400 p-[15px] pr-2 text-base font-normal leading-normal" placeholder="Nhập địa chỉ email của bạn" type="email" required value="{{ old('email') }}" />
...
<span class="truncate">Gửi Mã Xác Nhận</span>
</button>
</form>
</div>
<div class="mt-8 text-center">
<p class="text-[#618961] dark:text-gray-300 text-sm">
                    Nhớ mật khẩu của bạn? <a class="font-bold text-primary hover:underline" href="{{ route('login') }}">Quay lại Đăng Nhập.</a>
</p>
</div>
</div>
</div>
